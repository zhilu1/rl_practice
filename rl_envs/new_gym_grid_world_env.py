...
class GridWorldEnv(gym.Env):
    ...
    def __str__(self) -> str:
        ...
        for i in range(self.height):
            ...
            for j in range(self.width):
                state = (i, j)
                if state in self.forbidden_grids:
                    to_print += "{:2.2f}".format(self.forbidden_reward)
                elif state in self.target_grids:
                    to_print += "{:2.2f}".format(self.target_reward)
                else:
                    to_print += "{:2.2f}".format(0)
                to_print += " "
            to_print += "]\n"
        return to_print
